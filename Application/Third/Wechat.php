...
class Wechat
{
	...
	public function getAccess_token(){
		...
		$access_token = $cache->get("global::".$this->appid."_access_token");
		if($access_token){
			...
			if ($result)
			{
				...
				$cache->set("global::".$this->appid."_access_token", $this->access_token,$expire);
				return $this->access_token;
			}
			return false;
		}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		$cache = new CacheAdapter('memcache');
		$cache = $cache->getCacheAdapter();
		$access_token = $cache->get("global::".$appid."_access_token");
		if($access_token){
			$this->access_token = $access_token;
			return $this->access_token;
		}else{
			...
			if ($result)
			{
				...
				$this->access_token = $json['access_token'];
				$expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
				$cache->set("global::".$appid."_access_token", $this->access_token,$expire);
				return $this->access_token;
			}
			return false;
		}
	...
	public function getSignPackage($http_host="") {
		...
		$signature = $this->getSignature($arrdata);
	...
	private function getJsApiTicket() {
		...
		$ticket = $cache->get("global::".$this->appid."_jsapi_access_token");
		...
		if (!$ticket) {
			...
			// $url = "https://qyapi.weixin.qq.com/cgi-bin/get_jsapi_ticket?access_token=$accessToken";
				...
				$ticket = $res['ticket'];
				if ($ticket) {
					$cache->set("global::".$this->appid."_jsapi_access_token", $ticket,$res['expires_in']-200);
				}
			}
